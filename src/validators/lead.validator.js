...
export const createLeadValidator = [
  ...
  body('status').optional().isIn(LEAD_STATUS_VALUES).withMessage(`Status must be one of: ${LEAD_STATUS_VALUES.join(', ')}`),
  body('type').isIn(LEAD_TYPE_VALUES).withMessage(`Type must be one of: ${LEAD_TYPE_VALUES.join(', ')}`),
...
export const updateLeadValidator = [
  ...
  body('status').optional().isIn(LEAD_STATUS_VALUES).withMessage(`Status must be one of: ${LEAD_STATUS_VALUES.join(', ')}`),
  body('type').optional().isIn(LEAD_TYPE_VALUES).withMessage(`Type must be one of: ${LEAD_TYPE_VALUES.join(', ')}`),
  body('budget').optional({ checkFalsy: true }).isFloat({ min: 0 }).withMessage('Budget must be a positive number'),
  body('source').optional().trim(),
  body('location').optional().trim(),
  body('remarks').optional().trim(),
...
export const updateLeadStatusValidator = [
  param('id').isMongoId().withMessage('Invalid lead id'),
  body('status').isIn(LEAD_STATUS_VALUES).withMessage(`Status must be one of: ${LEAD_STATUS_VALUES.join(', ')}`),
];

export const leadIdParamValidator = [param('id').isMongoId().withMessage('Invalid lead id')];
